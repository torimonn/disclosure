...
6. 各表の金額単位に基づいて数値を補正し、貸借対照表→損益計算書の順に一列へまとめ
   つつ3列目に貸借対照表ページ、4列目に損益計算書ページを記録したCSVを生成
...
- 関数 `export_financials_to_csv()` により、最新会計年度の `end_date` を 2 行目・B 列に、会社名を 1 行目・B 列にセット。3 行目以降では貸借対照表項目の後に損益計算書項目を続けて並べ、金額は各表の単位を掛け合わせて円換算した値を保存します
- CSV は4列構成で、C列に貸借対照表ページ、D列に損益計算書ページが入ります。出力ファイル名は PDF 名を元に自動で決定されます
...
- 1 行目：A列 空白、B列 会社名、C列 空白、D列 空白
- 2 行目：A列 空白、B列 会計年度末日（YYYY-MM-DD）、C列 空白、D列 空白
- 3 行目以降：A列 勘定科目、B列 金額（円）、C列 貸借対照表ページ、D列 損益計算書ページ
- 貸借対照表項目の行では C 列にページ番号を、損益計算書項目の行では D 列にページ番号を出力します
- 抽出した `balance_sheet_amount_unit` もしくは `income_statement_amount_unit` を乗算し円換算した値を保存します。金額が `null` の場合は空文字として出力します
